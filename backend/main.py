...
def transcribe_audio(audio, lang='English'):
    ...
    translation_segments = model.transcribe(
        audio, **translate_options)["segments"]
    transcript_seg = []
    for i in range(len(translation_segments)):
        transcript_seg.append('[' + str(round(translation_segments[i]['start'], 2))+'-'+str(
            round(translation_segments[i]['end'], 2)) + ']' + translation_segments[i]['text'])
...
def split_tokens_chat(transcript_seg, type):
    ...
    if type == 'sentiment':
        prompt_request = "Analyze the sentiment of the comments of the YouTube video. Provide a summary of the overall sentiment." + str(prompt_response)
...
@app.post('/ask_GPT')
async def ask_GPT(ask_gpt: ask_GPT):
    ...
    if num_tokens > 2000:
        reply = split_tokens_chat(
            transcript_seg, ask_gpt.question)
    ...
    data = {'data': reply, 'message': 'Question answered successfully',
            'status_code': '200'}
...
async def transcribeAndStore(transcribe_model: transcribe):
    ...
    if transcribe_model.language == "English" or transcribe_model.language == "english":
        transcription = openai.Audio.transcribe(
            model="whisper-1", file=input_data, response_format='text')
    else:
...
async def sentiment_analysis(url: videoUrl):
    video_url = url.link
    ...
    num_tokens = count_tokens(comments)
    question = 'Analyze the sentiment of the comments of the YouTube video. Provide a summary of the overall sentiment.'
    if num_tokens > 2000:
        reply = split_tokens_chat(comments, "sentiment")
    else:
        message_history = []
        # reply = chat(transcript_seg, "What are all the main topics and their corresponding start and end time stamps in the text?",message_history )
        reply = chat(comments, question, message_history)

    data = {'data': reply, 'message': 'Sentiment analysis done successfully',
            'status_code': '200'}

    return data

@app.post('/textToSpeech')
async def TextToSpeech(transcribe_model: transcribe):
    print("Transcribe Endpoint Called")
    ...
    tts = gtts.gTTS(transcription, lang='en')
    tts.save('audio_translated.mp3')

    data = {'message': 'Audio translated successfully','status_code': '200'}
